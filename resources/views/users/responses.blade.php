...
<div id="right_container">
    <div style="padding:20px 15px 30px 15px;">
        ...
        <tbody>
            ...
            $sender_surname = User::getSurname($response->sender_id);
            ...
            <tr align="center">
                ...
                @endif 
                <td class="rel1-{{ $response->id }}" {{ $bold_response }}>{{ $sender_name }} {{ $sender_surname }}</td>
                <td class="rel1-{{ $response->id }}" {{ $bold_response }}>{{ $sender_city }}</td>
                <td class="rel1-{{ $response->id }}" {{ $bold_response }}>{{ substr($response->message,0,15) }}<a title="View Details" 
                    href="#responseDetails{{ $response->id }}" data-toggle="modal">...</a></td>
                <td class="rel1-{{ $response->id }}" {{ $bold_response }}>{{ $response->created_at }}</td>
                <td>
                    <a class="updateResponse" rel="{{ $response->id }}" title="View Details" href="#responseDetails{{ $response->id }}" 
                    ...
                    <a href="{{ url('contact/'.$sender_username.'?encoded_message='.$encoded_message) }}" 
                    target="_blank" title="Reply"><i class="fa fa-reply" aria-hidden="true"></i></a>&nbsp;
                    <a rel="{{ $response->id }}" rel1="delete-response" class="deleteAction" href="javascript:" title="Delete Message">
